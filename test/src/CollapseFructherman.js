...
const mapValueToProp = (items, field, prop, range) => {
  const minValue = Math.min(...items.map(item => item[field]));
  const maxValue = Math.max(...items.map(item => item[field]));
  items.forEach(item => {
    item[prop] = ((item[field] - minValue) / (maxValue - minValue)) * (range[1] - range[0]) + range[0];
  });
};
  ...
  if (model.id.startsWith('cluster')) {
    ...
    graphEdges.forEach(function (ge) {
      ...
      if (
        ...
        ge.hide();
        if (ge.get('sourceNode').get('visible') && targetModel.cluster !== model.cluster) {
          let _c = beginColor,
            _c2 = endColor;
          if (sourceModel.x > center.x) {
            _c = endColor;
          ...
          graph.addItem('edge', {
            ...
            style: {
              ...
              } else if (ge.get('targetNode').get('visible') && sourceModel.cluster !== model.cluster) {
              let _c = beginColor,
              _c2 = endColor;
              if (center.x > targetModel.x) {
              _c = endColor;
