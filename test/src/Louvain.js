...
const LouvainGraph = () => {
    ...
    const initGraph = new G6.Graph({
      ...
      defaultNode: {
        labelCfg: {
          ...
          position: 'center', // Position the label in the center of the node
        },
      },
    });
  ...
  useEffect(() => {
    if (graph) {
      ...
      const clusterColors = generateColorPalette(result.clusters.length);

      // Store original positions and update nodes with cluster information
      data.nodes.forEach(node => {
        const cluster = result.clusters.find(cluster => cluster.nodes.some(n => n.id === node.id));
        if (cluster) {
          node.style = {
            fill: clusterColors[cluster.id - 1],
            stroke: '#333',
          };
          node.label = `Cluster ${cluster.id}`;
...
function generateColorPalette(numColors) {
  ...
  for (let i = 0; i < numColors; i++) {
    const color = `hsl(${(i * 360 / numColors) % 360}, 100%, 70%)`;
    colors.push(color);
  }
  return colors;
}

export default LouvainGraph;